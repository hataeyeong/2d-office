...
export class Character {
    constructor(config) {
        ...
        // 충돌 박스 (발 위치 기준 - 64px 높이 캐릭터)
        this.collisionBox = {
            offsetX: 8,
            offsetY: 56,  // 64 - 8 = 발 근처
            width: 16,
            height: 8
        };
    }
    ...
    startTileMove(direction, dirOffsets, collisionCheck) {
        ...
        // 충돌 체크
        if (collisionCheck && collisionCheck(nextX, nextY, this)) {
            // 충돌 - 이동 불가, 방향만 바라봄
            this.direction = direction;
            this.animation.setDirection(direction);
            this.animation.setAnimation('idle', this.direction);
    ...
    render(ctx) {
        ...
        // 화면 좌표 (발 위치 기준으로 그리기 위해 Y에서 높이 절반 빼기)
        const destX = Math.floor(this.x);
        const destY = Math.floor(this.y) - (FRAME_HEIGHT - 32);  // 발 위치 기준 정렬
