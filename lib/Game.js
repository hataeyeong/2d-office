...
export class Game {
    ...
    render() {
        ...
        // 1. 베이스 레이어 렌더링 (바닥, 벽, 가구, 장식 - 캐릭터 아래)
        if (this.mapRenderer) {
            this.mapRenderer.renderBaseLayers(this.mapLayers);
        }

        // 2. 캐릭터들을 Y 좌표 순으로 정렬하여 렌더링 (깊이 정렬)
        const allCharacters = [...this.npcs];
        if (this.player) allCharacters.push(this.player);
        ...
        for (const character of allCharacters) {
            ...
            if (this.debug) {
                character.renderDebug(this.ctx);
            }
        }

        // 3. Front 레이어 렌더링 (캐릭터 위 - 책상, 테이블 등)
        if (this.mapRenderer) {
