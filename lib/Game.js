...
export class Game {
    ...
    render() {
        ...
        // 디버그: 60프레임마다 렌더링 순서 출력
        if (this.frameCount % 60 === 0 && this.player) {
            console.log(`[Render] 플레이어 Y=${this.player.y.toFixed(1)}`);
            console.log('[Render] 정렬된 엔티티:', ySortedEntities.map(e => ({
                type: e.type,
                sortY: e.sortY,
                tileType: e.entity?.type || 'char'
            })));
        }
